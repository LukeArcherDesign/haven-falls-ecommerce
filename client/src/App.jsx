...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      const syncCampfire = async () => {
        try {
          const response = await fetch("http://localhost:5001/api/auth/campfire/sync", {
            method: "POST",
            headers: {
  ...
  const addToKit = (product) => {
    ...
    // Start new countdown if new product added
  ...
  return (
    <Router>
      ...
      <Routes>
        ...
        <Route 
          path="/shop" 
          element={<Shop addToKit={addToKit} toggleCampfire={toggleCampfire} campfireList={campfireList} />} 
        />
        <Route 
          path="/shop/:id" 
          element={<ProductDetail addToKit={addToKit} toggleCampfire={toggleCampfire} campfireList={campfireList} />} 
        />
        <Route path="/contact" element={<Contact />} />
        <Route 
          path="/campfire" 
          element={<Campfire campfireList={campfireList} toggleCampfire={toggleCampfire} addToKit={addToKit} />} 
        />
