...
interface KitItem extends ShopItem {
  quantity: number;
}
...
const Cart = ({ kitItems, updateQuantity, removeFromKit }: CartProps) => {
  const total = kitItems.reduce(
    (sum: number, item: KitItem) => sum + item.price * item.quantity,
    0,
  );

  // Tell state it can accept string, number, or null
