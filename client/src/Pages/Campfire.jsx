...
const Campfire = ({ campfireList = [], toggleCampfire, addToKit }) => {
  return (
    <div className="campfire-wrapper">
      ...
      {/* Trapeze Header */}
      <div className="campfire-header-accent">
        <h1><TbCampfire className="header-icon" /> Campfire</h1>
      </div>
      ...
      {campfireList.length === 0 ? (
        <div className="empty-campfire-state">
          ...
          <p>No items around the campfire. Select your favourite items in the field to see them here.</p>
          <Link to="/shop" className="return-btn">Return to Field</Link>
        </div>
      ) : (
            ...
            <div key={item.id} className="shop-item-card">
              ...
              <div className="item-main-content">
                ...
                <Link to={`/shop/${item.id}`} className="product-portal-link">
                  <img src={`http://localhost:5001${item.image[0]}`} alt={item.name} />
                </Link>
              ...
              <div className="item-action-row">
                <button className="add-to-kit-btn" onClick={() => addToKit(item)}>
                  Add to Kit
                </button>
                <button 
